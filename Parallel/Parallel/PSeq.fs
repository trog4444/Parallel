...
module PSeq =    
    ...
    let inline append (xs: ^T pseq) (ys: ^T pseq) : ^T pseq = xs.Concat(ys)
    ...
    let inline cache (source: ^T pseq) : ^T pseq =
        match source with
    ...
    let inline choose (chooser: ^T -> ^U option) (source: ^T pseq) : ^U pseq = 
        source
            .Select(chooser)
            .Where(Option.isSome)
            .Select(Option.get)
    ...
    let inline collect (projection: ^T -> ^U seq) (source: ^T pseq) : ^U pseq =
        source.SelectMany(fun a -> match projection a with null -> Seq.empty | b -> b)
    ...
    let inline collectMap (projection: ^T -> ^U seq) (mapping: ^T -> ^U -> ^V) (source: ^T pseq) : ^V pseq =
        let nx xs = match xs with null -> Seq.empty | _ -> xs
        source.SelectMany(projection >> nx, mapping)    

    /// <summary>Combines the given enumeration-of-enumerations as a single concatenated enumeration.</summary>
    let inline concat (source: pseq<#seq< ^T>>) : ^T pseq =
        source.SelectMany(fun a -> match a with null -> Seq.empty | _ -> a :> _ seq)

    /// <summary>Evaluates to true if the given element is in the input sequence.</summary>
    let inline contains elem (source: ^T pseq) : bool =
        source.Any((=) elem)
    ...
    let inline countBy (f: ^T -> ^Key) (source: ^T pseq) : (^Key * int) pseq = 
        source.GroupBy(f).Select(fun kvs -> kvs.Key, kvs.Count())
    ...
    let inline distinct (source: ^T pseq) : ^T pseq =
        source.Distinct(HashIdentity.Structural)    
    ...
    let inline distinctBy (projection: ^T -> ^Key) (source: ^T pseq) : ^T pseq = 
        let comparer = 
    ...
    let inline exists (predicate: ^T -> bool) (source: ^T pseq) = 
        source.Any(predicate)

    /// <summary>Tests if any pair of elements of the input sequences satisfies the given predicate.</summary>
    let inline exists2 (predicate: ^T -> ^U -> bool) (source1: ^T pseq) (source2: ^U pseq) : bool = 
        source1.Zip(source2, predicate).Any(id)
    ...
    let inline filter (predicate: ^T -> bool) (source: ^T pseq) : ^T pseq =
    ...
    let inline fold (folder: ^State -> ^T -> ^State) (state: ^State) (source: ^T pseq) : ^State =
        source.Aggregate(state, folder)

    /// <summary>Tests if all elements of the sequence satisfy the given predicate.</summary>
    let inline forall (predicate: ^T -> bool) (source: ^T pseq) = 
    ...
    let inline forall2 (predicate: ^T -> ^U -> bool) (source1: ^T pseq) (source2: ^U pseq) : bool = 
        source1.Zip(source2, predicate).All(id)
    ...
    let inline groupBy (projection: ^T -> ^Key) (source: ^T pseq) : (^Key * ^T seq) pseq =
        source.GroupBy(projection, fun k v -> k, (match v with null -> Seq.empty | _ -> v))
    ...
    let inline join (outerSelector: ^T -> ^Key) (innerSelector: ^U -> ^Key) (resultSelector: ^T -> ^U -> ^V)
        (inner: ^U pseq) (outer: ^T pseq) : ^V pseq =
    ...
    let inline iter (action: ^T -> unit) (source: ^T pseq) : unit =
        source.ForAll(fun a -> action a)

    /// <summary>Applies the given function to two collections simultaneously.
    /// If one sequence is shorter than the other then the remaining elements of the longer sequence are ignored.</summary>
    let inline iter2 (action: ^T -> ^U -> unit) (source1: ^T pseq) (source2: ^U pseq) : unit =
        ignore ((source1).Zip(source2, action))
    ...
    let inline iteri (action: int -> ^T -> unit) (source: ^T pseq) : unit =
        ignore ((source).Select(fun t i -> action i t))

    /// <summary>Returns the length of the sequence.</summary>
    let inline length (source: ^T pseq) = source.Count()
    ...
    let inline map (mapping: ^T -> ^U) (source: ^T pseq) : ^U pseq =
    ...
    let inline map2 (mapping: ^T -> ^U -> ^V) (source1: ^T pseq) (source2: ^U pseq) : ^V pseq =
        source1.Zip(source2, mapping)
    ...
    let inline mapi (mapping: int -> ^T -> ^U) (source: ^T pseq) : ^U pseq =
        source.Select(fun t i -> mapping i t)
    ...
    let inline readonly (source: ^T pseq) : ^T pseq =
    ...
    let inline skip count (source: ^T pseq) : ^T pseq = 
        source.Skip(max 0 count)
    ...
    let inline skipWhile (predicate: ^T -> bool) (source: ^T pseq) : ^T pseq = 
        source.SkipWhile(predicate)

    /// <summary>Yields a sequence ordered by keys.</summary>
    let inline sort (source: ^T pseq) : ^T pseq when ^T : comparison = 
        source.OrderBy(id, ComparisonIdentity.Structural) :> ^T pseq
    ...
    let inline sortBy (projection: ^T -> ^Key when ^Key: comparison) (source: ^T pseq) : ^T pseq = 
        source.OrderBy(projection, ComparisonIdentity.Structural) :> ^T pseq

    /// <summary>Applies a key-generating function to each element of a sequence and yield a sequence ordered by keys.
    /// The keys are compared, descending, using generic comparison as implemented by Operators.compare.</summary>
    let inline sortByDescending (sorter: ^T -> ^Key) (source: ^T pseq) : ^T pseq =
        source.OrderByDescending(sorter, ComparisonIdentity.Structural) :> ^T pseq
    ...
    let inline tail (source: ^T pseq) : ^T pseq = source.Skip(1)

    /// <summary>Returns a sequence that when enumerated returns no more than a specified number of elements.</summary>
    let inline take count (source: ^T pseq) : ^T pseq =
        source.Take(max 0 count)
    ...
    let inline tryFindIndex (predicate: ^T -> bool) (source: ^T pseq) : int option =
        source
            .Select(fun t i -> struct (i, predicate t))
            .Where(fun struct (_, b) -> b)
            .Select(fun struct (v, _) -> Some v)
            .FirstOrDefault()

    /// <summary>Returns the first element of the sequence, or None otherwise.</summary>
    let inline tryHead (source: ^T pseq) : ^T option =
        source.Select(Some).FirstOrDefault()
    ...
    let inline tryMax (source: ^T pseq) : ^T option =
        if isNull source || Seq.isEmpty source then None else Some (source.Max())
    ...
    let inline tryMaxBy (projection : ^T -> ^U when ^U: comparison) (source : pseq< ^T>) : ^T option = 
        if isNull source || Seq.isEmpty source then None
        else Some (match source
    ...
    let inline tryMin (source: ^T pseq) : ^T option =
        if isNull source || Seq.isEmpty source then None else Some (source.Min())
    
    /// <summary>Returns the lowest of all elements of the sequence, compared by using Operators.min on
    /// the function result.</summary>
    let inline tryMinBy (projection: ^T -> ^U when ^U: comparison) (source: ^T pseq) = 
        if isNull source || Seq.isEmpty source then None
        else Some (match source
                    .Select(fun x -> struct (projection x, x))
                    .Aggregate(fun (struct (minVal, minElem)) (struct (curVal, curElem)) ->
    ...
    let inline tryNth n (source: ^T pseq) : ^T option =
        source.Skip(max 0 n).Select(Some).FirstOrDefault()
    ...
    let inline tryPick (chooser: ^T -> ^U option) (source: ^T pseq) : ^U option =
        source
            .Select(chooser)
            .Where(Option.isSome)
            .FirstOrDefault()
    ...
    let inline where (predicate: ^T -> bool) (source: ^T pseq) : ^T pseq = filter predicate source
    ...
    let inline zip (source1: ^T pseq) (source2: ^U pseq) : (^T * ^U) pseq =
